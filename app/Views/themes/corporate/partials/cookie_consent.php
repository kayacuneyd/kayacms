...
<style>
    ...
    #ck-consent .wrap { display: flex; align-items: center; justify-content: space-between; gap: 16px; flex-wrap: wrap; max-width: 1200px; margin: 0 auto; }
    #ck-consent p { margin: 0; flex: 1 1 420px; }
    #ck-consent .ck-c-actions { display: flex; gap: 8px; }
    #ck-consent button { border: 2px solid #fff; background: none; color: #fff; padding: 8px 18px; cursor: pointer; font-weight: 600; }
    #ck-consent button:hover { background: #fff; color: #000; }
</style>
<div id="ck-consent" role="dialog" aria-label="Cookie consent" hidden>
    <div class="wrap">
        <p>
            ...
            <?php if ($policyUrl !== ''): ?>
                <a href="<?= esc($policyUrl) ?>" style="color:#fff;text-decoration:underline" target="_blank" rel="noopener">Privacy Policy</a>
            <?php endif; ?>
        </p>
